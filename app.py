...
st.set_page_config(page_title="목재 재단 프로그램 Fast v12", layout="wide")
...
def parse_plan_workbook(file, forced_header_row=None, forced_product_col=None, forced_color_col=None):
    ...
    for sheet in xls.sheet_names:
        ...
        if raw.empty:
            continue
        hdr = forced_header_row if forced_header_row is not None else detect_date_header_row(raw)
        if hdr is None or hdr >= len(raw):
            parse_log.append({"sheet": sheet, "status": "skip", "reason": "날짜 헤더 행을 찾지 못함"})
            continue
        product_col = forced_product_col if forced_product_col is not None else 0
        if product_col >= len(raw.columns):
            parse_log.append({"sheet": sheet, "status": "skip", "reason": "품목코드 열 인덱스 범위 초과"})
            continue
        color_col = forced_color_col
        if color_col is not None and color_col >= len(raw.columns):
            color_col = None
        date_cols = []
        ...
        if not date_cols:
            parse_log.append({"sheet": sheet, "status": "skip", "reason": "월~토 날짜 열을 찾지 못함"})
            continue
        current_product = ""
        added = 0
        for r in range(hdr + 1, len(raw)):
            ...
            if not current_product:
                continue
            color = normalize_text(raw.iat[r, color_col]) if color_col is not None else ""
            for c, d in date_cols:
                ...
                if qty > 0:
                    ...
                    added += 1
        parse_log.append({"sheet": sheet, "status": "ok" if added > 0 else "empty", "header_row": hdr, "product_col": product_col, "color_col": color_col, "read_rows": added})
    ...
    log_df = pd.DataFrame(parse_log) if parse_log else pd.DataFrame(columns=["sheet", "status", "reason"])
...
st.title("목재 재단 프로그램 Fast v12")
st.caption("주차별 생산계획 해석 고정형 보정 버전")
...
if bom_df is not None and plan_file is not None:
    st.subheader("생산계획 해석 설정")
    c1, c2, c3 = st.columns(3)
    forced_header_row = c1.number_input("날짜 헤더 행 번호 (0부터)", min_value=0, value=2, step=1)
    forced_product_col = c2.number_input("품목코드 열 번호 (0부터)", min_value=0, value=0, step=1)
    forced_color_col = c3.number_input("색상 열 번호 (0부터, 없으면 큰 값 입력)", min_value=0, value=2, step=1)

    plan_df, parse_log_df = parse_plan_workbook(plan_file, int(forced_header_row), int(forced_product_col), int(forced_color_col))

    if plan_df.empty:
        st.warning("주차별 생산계획 해석 결과가 없습니다. 아래 해석 로그를 보고 행/열 번호를 조정해 주세요.")
    else:
    ...
    if not plan_df.empty:
        ...
        kerf = k1.number_input("톱날폭(mm)", min_value=0.0, value=4.8, step=0.1, format="%.1f")
        margin = k2.number_input("여유치(mm)", min_value=0.0, value=10.0, step=0.1, format="%.1f")
        rotate_allowed = k3.checkbox("회전 허용", value=True)
        ...
        for d, g in plan_df.groupby("date"):
            ...
            for _, row in g.iterrows():
                ...
                if color:
                    m2 = matched[matched["color"] == color].copy()
                    if not m2.empty:
                        matched = m2
                if matched.empty:
                    continue
            ...
            for grp, gg in part_df.groupby(["color", "thickness_mm"], dropna=False):
                ...
                daily_rows.append({"date": d, "color": grp[0], "thickness_mm": grp[1], "총 재단 수량": int(gg["qty"].sum()), "사용 원장 수": sub["used_boards"], "수율(%)": sub["yield_rate"], "자투리 면적": sub["waste_area"]})
            ...
            analysis_rows.append({"date": d, "추천 시나리오": best_row["시나리오"], "추천 수율(%)": best_row["수율(%)"], "추천 사용 원장 수": best_row["사용 원장 수"], "분석 요약": summary})

        daily_df = pd.DataFrame(daily_rows, columns=DAILY_RESULT_COLS)
        if daily_df.empty:
            st.warning("BOM과 생산계획이 매칭되지 않았습니다. 생산계획 해석 열 번호 또는 제품코드/색상 값을 확인해 주세요.")
        else:
